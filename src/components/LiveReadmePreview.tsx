...
import { Copy, Check, Eye, Moon, Sun, Monitor, Code2, Terminal, FileText } from 'lucide-react';
...
const CATEGORY_ORDER = ['frontend', 'backend', 'languages', 'mobile', 'databases', 'devops', 'marketing'];
...
export const LiveReadmePreview: React.FC<LiveReadmePreviewProps> = ({
  ...
  // Group selected badges by category
  const categorizedGroups = CATEGORY_ORDER.map((catKey) => {
    const badges = selectedBadgeObjects.filter((b) => b.category === catKey);
    return {
      key: catKey,
      meta: CATEGORY_MAP[catKey] || { label: catKey, icon: '⚡' },
      badges,
    };
  }).filter((group) => group.badges.length > 0);

  // Generate Categorized Markdown/HTML Code
  const generateCategorizedCode = (format: 'left-html' | 'centered-html' | 'multiline-md' | 'inline-md') => {
    if (categorizedGroups.length === 0) {
      return `<!-- Select technology badges above to see your categorized stack output -->`;
    }
  ...
  const fullReadmeSnippet = generateCategorizedCode(basketFormat);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-lg font-bold">Categorized Badges Preview</h3>
            ...
            <p className={`text-xs ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
              Selected technologies are automatically grouped by category (Frontend, Backend, Databases, CI/CD, Hosting).
            </p>
          </div>
        </div>
      ...
      <div className={`rounded-xl border shadow-2xl overflow-hidden transition-all duration-300 ${themeStyles.border} ${themeStyles.bg}`}>
        ...
        <div className={`px-4 py-3 border-b flex items-center justify-between ${themeStyles.headerBg} ${themeStyles.border}`}>
          <div className="flex items-center gap-2">
            ...
            <span className={`text-xs font-mono font-bold ${themeStyles.textHeading}`}>
              README.md — Categorized Tech Badges
            </span>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <span className="text-[11px] text-slate-400 font-mono hidden sm:inline">Align:</span>
              <select
                value={basketFormat}
            ...
            <button
              onClick={() => onCopyCode(fullReadmeSnippet, 'categorized-badges-code')}
            ...
            >
              {copiedId === 'categorized-badges-code' ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
              <span>{copiedId === 'categorized-badges-code' ? 'Copied Stack!' : 'Copy Categorized Stack'}</span>
            </button>
          </div>
        </div>
        ...
        <div className={`p-6 sm:p-8 font-sans ${themeStyles.textPrimary}`}>
          {showRawMarkdown ? (
            ...
            /* Rendered Categorized Badges Preview */
            <div className="space-y-6">
              {categorizedGroups.length > 0 ? (
                categorizedGroups.map((group) => (
                  <div key={group.key} className="space-y-2">
                    <h4 className={`text-base font-bold flex items-center gap-2 ${themeStyles.textHeading}`}>
                ...
                <div className={`p-8 rounded-xl border border-dashed text-center space-y-2 ${themeStyles.codeBg} ${themeStyles.border}`}>
                  ...
                  <p className="text-[11px] text-slate-500">
                    Select technology badges from Section 1 above (e.g. HTML, CSS, React, CodeIgniter 4, MySQL, Git, Vercel) to see them automatically categorized here!
                  </p>
                </div>
              )}
              ...
              {categorizedGroups.length > 0 && (
                <div className="pt-4 border-t border-slate-700/30 flex items-center justify-between text-[11px] text-slate-400 font-mono">
                  <span>⚡ Categorized Stack Output</span>
                  <span className="text-emerald-400 font-semibold">{selectedBadgeObjects.length} badges across {categorizedGroups.length} categories</span>
                </div>
              )}
            </div>
          )}
